...
{
	__maybe_unused struct sunxi_nfc *nfc =
				to_sunxi_nfc(chip->nand.controller);
	 ...
	 */
	chip->clk_rate = (2 * 1000000000L) / min_clk_period;
					      ...
					      struct nand_ecc_ctrl *ecc)
{
	...
	struct sunxi_nand_chip *sunxi_nand = to_sunxi_nand(nand);
	__maybe_unused struct sunxi_nfc *nfc =
				to_sunxi_nfc(sunxi_nand->nand.controller);
...
static int sunxi_nand_chip_init(int node, struct sunxi_nfc *nfc, int devnum)
{
	...
	for (i = 0; i < nsels; i++) {
		...
		chip->randomize =
			fdtdec_get_bool(blob, node, "allwinner,randomize");
	}
